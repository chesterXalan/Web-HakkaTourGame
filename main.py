...
from core.utils import get_google_cloud_key, get_openai_key, get_serp_key
...
def index():
    return render_template(
        "index.html", title=__title, description=__description, version=__version
    )
...
def game():
    return render_template(
        "game.html", title=__title, googleCloudKey=get_google_cloud_key()
    )
...
def game_startup(data):
    ...
    chain_game = langchain.set_chain(
        prompt=langchain.prompt_game(
            game_params["num_county"], game_params["num_attr"]
        ),
    ...
    chain_attr = langchain.set_chain(
        prompt=langchain.prompt_attraction(), llm=langchain.chatgpt()
    )
    user_dict[connect_time] = dict(
        game_idx=1,
...
def play(data):
    ...
    if user_data["game_idx"] == 1:  # 開始遊戲、回答題目
        ...
        user_data["game"].run(input=user_input)
        emit("game_tts_autoplay")
        ...
        user_data["game_map"], county_id, county_name = update_taiwan_map(
            user_data["game_map"],
            user_data["countyid_last"],
            user_data["countyid_visited"],
        )
        user_data["countyid_last"] = county_id
        user_data["countyid_visited"].append(county_id)

        emit("clear_output_text")
        response_game = user_data["game"].run(input=county_name)
        emit("game_tts_autoplay")
        user_data["attr_list"] = langchain.get_list_output(
            user_data["attr"].run(input=response_game)
        )
