...
class CountySelector():
    def __init__(self):
        ...
        self.counties = self.getCountyNames()
    ...
    def updateMap(self, root, rand_id):
        ...
        return self.showMap(root)
    
    def saveMap(self, root, fixed_id):
        elem_county = root.find(f"{self.svg_prefix}g[@id='county']")
        counties = [e for e in elem_county.iter(f"{self.svg_prefix}path")]
